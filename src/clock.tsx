  ...
  return (
    <Centering style={{ padding: "1rem" }}>
      <div style={{ display: "flex", justifyContent: "center" }}>
        <svg viewBox={`0 0 ${VB_WIDTH} ${VB_HEIGHT}`} style={{ width: VB_WIDTH, maxWidth: "min(90vw, 90vh)" }}>
          <circle cx={VB_WIDTH / 2} cy={VB_HEIGHT / 2} r={RADIUS} fill="none" stroke={COLOR} strokeWidth={BOLDNESS} />
          {!error && ageX && ageY && (
            <>
              <line x1={VB_WIDTH / 2}
                    y1={VB_HEIGHT / 2}
                    x2={(ageX * AGE_HAND_LENGTH_RATIO) + (VB_WIDTH / 2)}
                    y2={(VB_HEIGHT / 2) - (ageY * AGE_HAND_LENGTH_RATIO)}
                    stroke={COLOR}
                    strokeWidth={BOLDNESS}
                    stroke-linecap="round" />
              <line x1={VB_WIDTH / 2}
                    y1={VB_HEIGHT / 2}
                    x2={(yearX * YEAR_HAND_LENGTH_RATIO) + (VB_WIDTH / 2)}
                    y2={(VB_HEIGHT / 2) - (yearY * YEAR_HAND_LENGTH_RATIO)}
                    stroke={COLOR}
                    strokeWidth={BOLDNESS}
                    stroke-linecap="round" />
            </>
